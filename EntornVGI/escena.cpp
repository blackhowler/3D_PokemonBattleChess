...
// CONTROL POSICIÓ PECES: Dos matrius (int i bool per al control de la posició de cada peça)
...
void dibuixa_EscenaGL(char objecte, CColor col_object, bool ref_mat, bool sw_mat[4], bool textur)
{
	...
	switch (objecte)
	{
	// Dibuix del Taulell i les peces
		/*
	case BATTLECHESS:
...
void dibuixa_EscenaBC(int* tauler_o, int* tauler_p)
{
	tauler();
	fitxesBlanques(tauler_p, tauler_o);
	fitxesNegres(tauler_p, tauler_o);
}
...
void tauler(void)
{
	// CANTONADES TAULELL
	{
		// Casilla A8 (Torre esquerra negra)
		glPushMatrix();
			glTranslatef(-LIMITE_X, -LIMITE_Y, SUELO_Z);
			glRotatef(180.0f, 0.0f, 0.0f, 1.0f);
			glCallList(CAS_CANTO);
		glPopMatrix();

		// Casilla H8 (Torre dreta negra)
		glPushMatrix();
			glTranslatef(-LIMITE_X, LIMITE_Y, SUELO_Z);
			glRotatef(90.0f, 0.0f, 0.0f, 1.0f);
			glCallList(CAS_CANTO);
		...
		// Casilla A1 (Torre esquerra blanca)
		glPushMatrix();
			glTranslatef(LIMITE_X, -LIMITE_Y, SUELO_Z);
	...
	// MARGES TAULELL
	{
		// Marge superior (Fitxes traseres negres)
		for (int i = 1; i < MIDA_TAULER - 1; i++)
		{
			glPushMatrix();
				glTranslatef(-LIMITE_X, -LIMITE_Y + i*MIDA_CASELLA, SUELO_Z);
				glRotatef(90.0f, 0.0f, 0.0f, 1.0f);
				glCallList(CAS_MARGE);
			glPopMatrix();
		}

		// Marge inferior (Fitxes traseres blanques)
		for (int i = 1; i < MIDA_TAULER - 1; i++)
		{
			glPushMatrix();
				glTranslatef(LIMITE_X, -LIMITE_Y + i*MIDA_CASELLA, SUELO_Z);
		...
		{
			glPushMatrix();
			glTranslatef(-LIMITE_X + i*MIDA_CASELLA, -LIMITE_Y , SUELO_Z);
			glRotatef(180.0f, 0.0f, 0.0f, 1.0f);
			glCallList(CAS_MARGE);
			glPopMatrix();
		}
	...
	// CASELLES CENTRALS TAULELL
	{
		for (int i = 1; i < MIDA_TAULER-1; i++)
		{
			for (int j = 1; j < MIDA_TAULER-1; j++)
			{
				glPushMatrix();
					glTranslatef(-LIMITE_X + i*MIDA_CASELLA, -LIMITE_Y + j*MIDA_CASELLA, SUELO_Z);
					glCallList(CAS_CENTRAL);
				glPopMatrix();
			}
		}
	}
...
void fitxesBlanques(int* tauler_p, int* tauler_o)
{

	//TODO : Afegir color blanc a les fitxes

	for (int i = 0; i < MIDA_TAULER; i++)
	{
		for (int j = 0; j < MIDA_TAULER; j++)
		{
			if (tauler_o[i*MIDA_TAULER + j] == 1)
			{
				glPushMatrix();
					glTranslatef(-LIMITE_X + i*MIDA_CASELLA, -LIMITE_Y + j*MIDA_CASELLA, SUELO_Z);
					glRotatef(-90.0f, 0.0f, 0.0f, 1.0f);
					glCallList(tauler_p[i*MIDA_TAULER + j]);
				glPopMatrix();
			}
		}
	}
...
void fitxesNegres(int* tauler_p, int* tauler_o)
{
	//TODO : Afegir color negre a les fitxes
	for (int i = 0; i < MIDA_TAULER; i++)
	{
		for (int j = 0; j < MIDA_TAULER; j++)
		{
			if (tauler_o[i*MIDA_TAULER + j] == 2)
			{
				glPushMatrix();
					glTranslatef(-LIMITE_X + i*MIDA_CASELLA, -LIMITE_Y + j*MIDA_CASELLA, SUELO_Z);
					glRotatef(90.0f, 0.0f, 0.0f, 1.0f);
