...
BEGIN_MESSAGE_MAP(CEntornVGIView, CView)
	...
	ON_UPDATE_COMMAND_UI(ID_VISTA_SATELIT, &CEntornVGIView::OnUpdateVistaSatelit)
		ON_COMMAND(ID_OBJECTE_BATTLECHESS, &CEntornVGIView::OnObjecteBattlechess)
		ON_UPDATE_COMMAND_UI(ID_OBJECTE_BATTLECHESS, &CEntornVGIView::OnUpdateObjecteBattlechess)
		END_MESSAGE_MAP()
...
//	Dibuix geometria de l'escena amb comandes GL.
	if (objecte == BATTLECHESS) dibuixa_EscenaBC(tauler_o, tauler_p);
	else dibuixa_EscenaGL(objecte, col_obj, true, sw_material, textura);
...
void CEntornVGIView::Inicialitzar_Tauler()
{
	for (int i = 0; i < MIDA_TAULER; i++)
	{
		tauler_o[i] = 2;
		tauler_o[i + MIDA_TAULER] = 2;
		tauler_o[i + MIDA_TAULER*(MIDA_TAULER - 2)] = 1;
		tauler_o[i + MIDA_TAULER*(MIDA_TAULER - 1)] = 1;
		for (int j = 2; j < MIDA_TAULER - 2; j++)
		{
			tauler_o[i + MIDA_TAULER*j] = 0;
		}
	}

	int ordrePeces[MIDA_TAULER] = { TORRE, CAVALL, ALFIL, REINA, REI, ALFIL, CAVALL, TORRE };

		for (int i = 0; i < MIDA_TAULER; i++)
		{
			tauler_p[i] = ordrePeces[i];
			tauler_p[i + MIDA_TAULER] = PEO;
			tauler_p[i + MIDA_TAULER*(MIDA_TAULER - 2)] = PEO;
			tauler_p[i + MIDA_TAULER*(MIDA_TAULER - 1)] = ordrePeces[MIDA_TAULER - i - 1]; 
		}
...
void CEntornVGIView::OnObjecteBattlechess()
{
	// TODO: Agregue aquí su código de controlador de comandos
	objecte = BATTLECHESS;

	lecturaOBJ(SZ_ALFIL, ALFIL);
	lecturaOBJ(SZ_CAVALL, CAVALL);
	lecturaOBJ(SZ_PEO, PEO);
	lecturaOBJ(SZ_REINA, REINA);
	lecturaOBJ(SZ_REI1, REI);
	lecturaOBJ(SZ_REI2, REI2);
	lecturaOBJ(SZ_TORRE, TORRE);
	lecturaOBJ(SZ_CAS_CENTRAL, CAS_CENTRAL);
	lecturaOBJ(SZ_CAS_ESQUINA, CAS_CANTO);
	lecturaOBJ(SZ_CAS_MARGEN, CAS_MARGE);
...
void CEntornVGIView::OnUpdateObjecteBattlechess(CCmdUI *pCmdUI)
{
	// TODO: Agregue aquí su código de controlador de IU para actualización de comandos
	if (objecte == BATTLECHESS) pCmdUI->SetCheck(1);
		else pCmdUI->SetCheck(0);
}
...
void CEntornVGIView::lecturaOBJ(char *szFileName, unsigned int iDisplayList)
{
	wglMakeCurrent(m_pDC->GetSafeHdc(), m_hRC);	// Activem contexte OpenGL

	if (ObOBJ == NULL) ObOBJ = new COBJModel;
	ObOBJ->LoadModel(szFileName, iDisplayList);
