...
// BattleChess: OBJECTE GAME. Include per la definició de tots els objectes necessaris per a l'execució de l'aplicació.
...
// BattleChess: Objecte Tauler:
	int tauler_p[MIDA_TAULER * MIDA_TAULER];			// Matriu de l'ID de les peces que hi ha al taulell
	int tauler_o[MIDA_TAULER * MIDA_TAULER];			// Matriz de ocupació de les peces que hi ha al taulell (0: Buida; 1: Equip 1; 2: Equip 2)
...
public:
	...
	void CEntornVGIView::lecturaOBJ(char *szFileName, unsigned int iDisplayList);	// Llegeix un objecte a partir de la direcció i el precompila amb la ID iDisplayList
...
public:
	...
	afx_msg void OnObjecteBattlechess();
	afx_msg void OnUpdateObjecteBattlechess(CCmdUI *pCmdUI);
};
